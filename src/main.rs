...
async fn handle_socket(socket: WebSocket, _who: SocketAddr, state: Arc<AppState>) {
    ...
    *USER_ID.lock().unwrap() += 1;
    ...
    let _ = sender.send(Message::Text(serde_json::to_string(&RetrieveMessages {msgs: MESSAGES.lock().unwrap().clone().to_vec()}).expect("couldn't serialize MESSAGES vector!"))).await;
