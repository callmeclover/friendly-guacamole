...
pub fn into_censored_md(html: &str, user: &mut User) -> Result<String, BlockReason> {
    ...
    let mut nodes_text: Vec<String> = document.select_first("p").expect("").descendants().text_nodes().map(|text| {<RefCell<String> as Clone>::clone(&text).into_inner()}).collect();
    nodes_text.pop();
    ...
    for (index, text_node) in document.select_first("p").expect("").descendants().text_nodes().enumerate() {
